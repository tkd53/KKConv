...
#
# ����ˡ : CrossEntropyWithStochSegTextWordList.perl (STEP) (FILESTEM) [TEST]
#
# ��  �� : CrossEntropyWithStochSegTextWordList.perl 4 91 ../../corpus/TRL10.morpyomi
#
...
sub RCWordMarkov{
    ...
    my(@sum1) = (0) x $intstr->size();            # ��_w2 F(w1,w2)
    my(@sum2) = (0) x $intstr->size();            # ��_w1 F(w1,w2)
    my($sumT) = 0;                                # ��_w1,w2 F(w1,w2)
    my($nonz) = 0;                                # �󥼥����Ǥο�

    printf(STDERR "RCWordMarkov(size = %d) .", $intstr->size());

    $markov->rep($text->Freq());                  # F();
#    $w1 = $intstr->str($suf1);
#    printf(STDERR "freq(BT) = %f\n", $text->Freq("BT"));

#    my($freq) = $text->Freq();
#    printf(STDERR "Freq() = %f\n", $text->Freq());
#    foreach $suf1 (1 .. ($intstr->size()-1)){     # ������
#        $w1 = $intstr->str($suf1);
#        printf(STDERR "Freq(%s) = %f\n", $w1, $text->Freq($w1));
#        $freq -= $text->Freq($w1);                # ñ�� 1-gram �������� F(w1)
#    }
#    printf(STDERR "freq(UT) = %f\n", $freq);
#    exit(0);

    foreach $suf1 (1 .. ($intstr->size()-1)){     # ������
#        if ($suf1%int(1+$intstr->size()/10) == 0){
        if ($suf1%50 == 0){
            printf(STDERR " %d .", $suf1);
        }
        $w1 = $intstr->str($suf1);
        $freq = $text->Freq($w1);                 # ñ�� 1-gram �������� F(w1)
#        printf(STDERR "freq(%s) = %f\n", $w1, $freq);
        ($freq != 0) || next;
        $markov->rep($freq, $suf1);               # F(w1);
        foreach $suf2 (1 .. ($intstr->size()-1)){
            $w2 = $intstr->str($suf2);
            $freq = $text->Freq($w1, $w2);        # ñ�� 2-gram �������� F(w1,w2)
#            printf(STDERR "freq(%s,%s) = %f\n", $w1, $w2, $freq);
            ($freq != 0) || next;
            $markov->rep($freq, $suf1, $suf2);    # F(w1,w2)
            $sum1[$suf1] += $freq;
            $sum2[$suf2] += $freq;
            $sumT += $freq;
            $nonz++;
    ...
    foreach $suf1 (1 .. ($intstr->size()-1)){     # F(w1,UT) ���׻������롼��(������)
        $freq = $text->Freq($intstr->str($suf1)); # F(w1)
        $freq -= $sum1[$suf1];                    #       - ��_w2 F(w1,w2)
        ($freq != 0) || next;
        $markov->rep($freq, $suf1, 0);            #                        = F(w1,UT)
        $sumT += $freq;
        $nonz++;
    }

    foreach $suf2 (1 .. ($intstr->size()-1)){     # F(UT,w2) ���׻������롼��(������)
        $freq = $text->Freq($intstr->str($suf2)); # F(w2)
        $freq -= $sum2[$suf2];                    #       - ��_w1 F(w1,w2)
        ($freq != 0) || next;
        $markov->rep($freq, 0, $suf2);            #                        = F(UT,w2)
        $sumT += $freq;
        $nonz++;
    }
    ...
    $freq = $text->Freq();                        # F()
    foreach $suf1 (1 .. ($intstr->size()-1)){     # F(UT) ���׻������롼��
        $w1 = $intstr->str($suf1);
        $freq -= $text->Freq($w1);                #       - F(w1)
    }
    $markov->rep($freq, 0);                       # F(UT);

    printf(STDERR "�󥼥�Ψ = %d/%d = %5.2f%%\n", $nonz, $intstr->size()**2,
           100*$nonz/($intstr->size()**2));
...
#-------------------------------------------------------------------------------------
    ...
    printf(STDERR "\n%s\n", "-" x 86);
    printf(STDERR "ñ�� 2-gram �����å�\n");
    foreach $suf1 (0 .. ($intstr->size()-1)){     # ������
        $w1 = $intstr->str($suf1);
        printf(STDERR "F(%s) = %f = �� F(%s, *) = ",
               $w1, $markov->_1gram($suf1), $w1);
        my($sum) = 0;
        foreach $suf2 (0 .. ($intstr->size()-1)){
            $sum += $markov->_2gram($suf1, $suf2); # F(w1,w2)
        }
        printf(STDERR " %f", $sum);
        if (&myeq($sum, $markov->_1gram($suf1))){
            printf(STDERR " ... OK\n");
        }else{
            printf(STDERR " ... NG\n");
        }
    ...
    printf(STDERR "%s\n", "-" x 86);
    printf(STDERR "ñ�� 1-gram �����å�\n");
    my($sum) = 0;
    printf(STDERR "F() = %f = �� F(*) = ", $markov->_0gram());
    foreach $suf1 (0 .. ($intstr->size()-1)){     # ������
        $sum += $markov->_1gram($suf1);           # F(w1)
    }
    printf(STDERR " %f", $sum);
    if (&myeq($sum, $markov->_0gram())){
        printf(STDERR " ... OK\n\n");
    }else{
        printf(STDERR " ... NG\n\n");
    }
